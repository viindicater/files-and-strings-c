...
/*char **tokenize_line(const char *line, int *count){ // makes a list of tokens, using space as delimiter (except when wrapped in quotes, i.e. in line 'ab "cd ef"' the tokens are 'ab' and 'cd ef')
...
char *concatv(const char *first, ...){ // return a combined buffer of all passed strings (last arg must be NULL, and need to free( ) after)
	va_list v; size_t len = 0; const char *c;
	va_start(v, first); for(c = first; c != NULL; c = va_arg(v, const char*)){ len += strlen(c); } va_end(v);
	char *s = malloc(len + 1); if(!s){ puts("MEMORY ERROR."); return NULL; }
...
int str_to_int(const char *s, int *out){
	if(!s || *s == '\0'){ return 1; } // if NULL ptr or empty str
	char *end; errno = 0; long v = strtol(s, &end, 10);
	if(*end != '\0') return 1; // didn't reach string end, i.e. string has non numeric characters
	if(errno == ERANGE || v > INT_MAX || v < INT_MIN) return 2; // if v out of range then overflow, errno == ERANGE if strtol itself overflows long
...
int read_file(char *name, char ***lines, size_t *count){
	...
	if(f){
		...
		while((buffer = readline(f)) != NULL){
			...
			if(!tmp){ free(buffer); printf("MEMORY ERROR: failed to read '%s'.\n", name); error = 1; break; }
			else{
				*lines = tmp;
				(*lines)[(*count)++] = buffer;
			}
...
int main(int argc, char *argv[]){
	/*if(1){ // Test (reads input, tokenize_line( ) , then print if each token is a valid file or directory (and list all files if directory)
		...
		int n; char **tok = tokenize_line(line, &n); if(!tok){ puts("MEMORY ERROR (2)."); free(line); return 1; }
		for(int i = 0; i < n; i++){
	...
	}else{ puts("Error: failed to save."); }
